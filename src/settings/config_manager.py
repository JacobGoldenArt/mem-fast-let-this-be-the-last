...
class ConfigManager:
    ...
    Example:
        ...
        # Creating a config         = config_manager.create_config(thread_id="thread_123", user_id="user_456")
    ...
    def __init__(self, config_class: Type[TypedDict], defaults: dict):  # type: ignore
        self.config_class = config_class
        self.defaults = defaults

    def process_config(self, config: RunnableConfig) -> TypedDict:  # type: ignore
        """
        ...
        return self.config_class(
            **{
                k: configurable.get(k, self.defaults.get(k))
                for k in self.config_class.__annotations__
            }
        )
    ...
    def create_config(self, **kwargs) -> RunnableConfig:
        ...
        """
        return ensure_config({"configurable": {**self.defaults, **kwargs}})